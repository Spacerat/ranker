...
class App extends Component<Props, State> {
    ...
    return (
      <div>
        ...
        <div>
          <input
            type="text"
            value={this.state.itemspec}
            onChange={(e) => this.set_items(e.target.value)}
          />
        </div>
        <div>
        ...
        <div>
          <button
            onClick={() => this.setState({ state: this.state.state.undo() })}
          >
            {" "}
            Undo
          </button>
  ...
  set_items(text: string) {
    this.setState({
      state: this.state.state.set_items(text.split(" ").map((x) => x.trim())),
      itemspec: text,
    });
  }
